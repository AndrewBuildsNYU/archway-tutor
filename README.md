...
plus multi-turn state** â€” how much of a tutor's behaviour lives in the prompt rather than
...
Then open `index.html` â€” double-click it, or serve the folder with any static file
...
over the answer key. What holds is naming the failure modes explicitly â€” answering
outright, stacking three questions into one turn, lecturing, condescending â€” and fixing
...
- **tier 1** â€” a conceptual nudge, no method named
- **tier 2** â€” names the method or theorem and why it fits, without applying it
- **tier 3** â€” writes the first concrete step, then hands the pen back
...
advice. Model output is written with `textContent` only â€” never `innerHTML` â€” because a
